...
int leadingoneFitness(individual temp)
{
	number_of_evaluations++;
	return leadingone(temp.value);
}
...
void initialize_population(int n)
{
	for (int i = 0; i < n; i++)
	{
		...
		{
			population[i].value[j] = ((int)rand() % 2);
		}
		if (problem_type==0)
		{
			*population[i].fitness = onemaxFitness(population[i]);
		}
		if (problem_type==1)
		{
			*population[i].fitness = trap5Fitness(population[i]);
		}
		if (problem_type==2)
		{
			*population[i].fitness = leadingoneFitness(population[i]);
		}
	}
}
...
void one_point_Offspring(int n)
{
	for (int i = 0; i < n; i += 2)
	{
		...
		if (problem_type==0)
		{
			*offspring[i].fitness = onemaxFitness(offspring[i]);
			*offspring[i+1].fitness = onemaxFitness(offspring[i+1]);
		}
		if (problem_type==1)
		{
			*offspring[i].fitness = trap5Fitness(offspring[i]);
			*offspring[i+1].fitness = trap5Fitness(offspring[i+1]);
			
		}
		if (problem_type==2)
		{
			*offspring[i].fitness = leadingoneFitness(offspring[i]);
			*offspring[i+1].fitness = leadingoneFitness(offspring[i+1]);
		}
...
int main()
{
	...
	cin >> problem_size;
	cout << "Chon function Onemax hay trap5 hay leadingone: (0: onemax, 1: trap5, 2: leadingone): ";
	cin >> problem_type;
